...
- Recovery runs before traffic and is safe across workers: protect fresh bounded work and settle only stale requests. Use PostgreSQL TIMESTAMPTZ leases and the database clock, renew only active unexpired ownership, and bound background recovery with skip-locked batches. Never revive expired work or overwrite a terminal outcome. Preserve completed records and admitted user messages. Fail startup if storage/recovery fails; never reset on startup. Document explicit development database recreation for schema changes, with no migration framework. Container data-volume ownership belongs with Docker guidance.
