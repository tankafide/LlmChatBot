...
The approved baseline uses non-streaming request replay, thread-local complete database units, PostgreSQL multi-worker coordination, stale-only recovery, and completion only after commit. The skill links the current baseline rather than duplicating its lifecycle table. Architecture, persistence, API contracts, and verification retain their respective detailed ownership.
