...
For affected async paths, follow reachable blocking I/O and substantial CPU operations; verify execution boundaries, cancellation and resource ownership. For data access, inspect filters/order/limits, serialization-triggered lazy queries, N+1 growth and unbounded history/results. Check concurrency/deadline/retry budgets and evidence for stated capacity. Lint and typing do not establish event-loop responsiveness, bounded queries or scalability. Do not presume PostgreSQL alone proves horizontal scalability; verify cross-worker coordination and distinguish per-worker limits from global capacity.
