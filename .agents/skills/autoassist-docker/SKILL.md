...
Read [AGENTS.md](../../../AGENTS.md) and the [shared stack baseline](../../../docs/stack-baseline.md), including its exclusive-volume and chat lifecycle requirements. Inspect existing Compose files, Dockerfiles, ignore files, manifests, lockfiles, settings, and verification scripts before choosing commands. The baseline records accepted decisions; files establish what is implemented. `tech-stack` remains an explicit-only decision workflow, not an automatic prerequisite.
...
- Copy dependency manifests/locks before frequently changing source when useful for caching. Keep Python virtual environments and Node modules inside their containers. A broad source bind mount can hide image-installed dependencies; use selective source mounts or an explicit container dependency location. Account for stale dependency volumes after lock changes without deleting the SQLite volume.

## Storage, process, and connectivity invariants

- Mount the SQLite **directory** on a named volume, including its journal/WAL side files. Keep the database URL pointed there. Run the app as a non-root user and verify that user can create/write files in the mounted directory; image permissions alone do not prove an existing volume is writable.
- Preserve one backend process/worker and no overlapping backend instances against each data volume, including one-off commands that could run startup recovery. Do not apply generic multi-worker recommendations to this prototype. Startup must validate storage and complete repeatable recovery before admitting requests; no reset, reseed-overwrite, or in-memory fallback on failure.
...
For affected implementation, validate Compose, build from locks, and exercise the actual service through its published API. Use the project's one cross-platform verification entry point and the same entry point in CI; add checks there rather than creating competing automation. Propagate container/check exit codes to the caller and prove an intentional failing check returns failure. Never run tests against the development SQLite volume.

For persistence/shutdown changes, use a disposable, separately named test volume: write a conversation, recreate the container, and confirm messages and terminal replay survive. Include an abrupt-stop/in-progress recovery case with backend-verification once scaffolded. Wait for the old process to stop before starting its replacement. Distinguish application-object recreation evidence from process/container restart evidence. Coordinate with persistence/backend-verification on persisted outcomes and frontend/frontend-verification on browser connectivity.
